...
| [`@vilancer/web`](packages/web) (DOM renderer) | — | Private, unpublished. Real content shipped in Phase 4: a probing, never-throwing `fromJitsiConference` transport adapter over `lib-jitsi-meet`'s data channel, an SVG renderer mounted over jitsi-meet's `#largeVideo` (proving one remote stroke renders end to end, covered by a jsdom test), and a `try`/`catch`-wrapped bootstrap entry point. Deployment (the real `custom-config.js` mount, an IIFE bundle, a real Jibri recording) is a later phase's job. |
