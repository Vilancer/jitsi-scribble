...
| [`@vilancer/react-native`](packages/react-native) | — | Private, unpublished. Renamed from the workspace-scaffold placeholder `jitsi-scribble`. Toolchain (dependencies, `bob`-based build target, `jest`/`jest-expo` test target, `.tsx` lint coverage) now stands up end to end, plus a minimal real `<ScribbleOverlay>` placeholder and DRAW-03's tested tap/drag classifier (`classifyGesture`). Gesture capture, local-echo rendering, and remote stroke rendering land in later plans of this phase. |
